...
- Deterministic renderer: palm/chest sigils, hand/core plasma arcs, trails, aura, charge orb, ground glyph, gesture-triggered expansions.
- Palettes: arcane, solar, void, jade.
- TUI feature: `Cyber Mage`.

## Quality ladder
- **Prototype:** camera silhouette with magic-circle overlays.
- **Usable:** persistent anchors, smoothing, body-owned effects, clean fullscreen operation. **Current baseline.**
...
`camera -> segmentation/pose/hands/flow -> PerformerRig -> gesture/event state -> deterministic FX modules -> optional neural style skin -> projector warp/compensation`

Effects must consume semantic anchors, not raw pixel positions scattered through renderer code. A learned tracker may replace the anchor estimator without changing effect modules.
...
## Gesture vocabulary
Charge = hands together; summon/shield = arms spread; ascension = hands raised; cast/thrust = fast hand motion away from core; swipe = high tangential wrist velocity; spin = body-axis rotation; jump = feet/body centroid impulse; crouch = compact pose. Gestures need hysteresis, confidence, cooldowns and state transitions rather than one-frame booleans.
...
3. Persistent effect state and particle IDs.
...
- No live preset morph/state machine or audio fusion yet.
...
3. Split effect renderer into composable modules and expose per-module enable/intensity/scale/color controls.
4. Add SDF/GLSL rune/glyph atlas and particle field.
5. Implement gesture state machine with charge->hold->release and shield/summon/cast events.
...
## Metrics
Anchor jitter in pixels, reacquisition time, gesture precision/false trigger rate, motion-to-effect latency, effect ownership under fast motion, display FPS, neural keyframe age, temporal flicker, VRAM, projector registration error.
