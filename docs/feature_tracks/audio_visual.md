...
- Piano mode turns ordinary MIDI performance data into visual expression: pitch/color, note velocity/strike, polyphonic density, register spread, chord tension, sustain/soft pedals, aftertouch and bend. The low-key cue zone is optional and disabled by default.
- Beat/bar quantization, sparse musical cue-loop recording, localhost browser dashboard, OSC live-state broadcast and optional CC-only MIDI state feedback now sit on the same Director bus. Optional Ableton Link clocking can replace the audio tempo/phase clock without replacing audio energy/event analysis.
