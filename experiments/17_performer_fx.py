...
def _make_tracker(name: str, confidence: float, smoothing: float):
    if name == "rtmpose":
        return RTMPoseWholeBodyTracker(
            mode="lightweight",
            backend="onnxruntime",
            device="cpu",
    ...
    if name == "auto":
        errors: list[str] = []
        try:
            tracker = RTMPoseWholeBodyTracker(
                mode="lightweight",
                backend="onnxruntime",
                device="cpu",
                confidence=confidence,
        ...
        raise RuntimeError(
            "No semantic performer tracker is available. Install the fully-open RTMPose path "
            "with `python -m pip install -e '.[performer]'` or the optional MediaPipe backend "
            "with `python -m pip install -e '.[mediapipe]'`. " + " | ".join(errors)
        )
...
def _anchor_emitters(state, style: str, intensity: float) -> list[ParticleEmitter]:
    ...
    specs = (
        ("left_palm", 0.05, 1.00, 0.020),
        ("right_palm", 0.62, 1.00, 0.020),
        ("chest", 0.80, 0.62, 0.026),
        ("head", 0.48, 0.36, 0.014),
        ("left_ankle", 0.26, 0.34, 0.013),
        ("right_ankle", 0.70, 0.34, 0.013),
    )
    if style == "bio":
    ...
    for name, hue, base_energy, radius in specs:
        ...
        emitters.append(
            ParticleEmitter.from_anchor(
                a,
                energy=(base_energy + speed * 0.55) * intensity,
                hue=hue,
                radius=radius * (1.0 + speed * 0.75),
            )
        )
    return emitters[:8]
    ...
    try:
        with Camera(args.camera, args.capture_width, args.capture_height) as cam:
            while True:
                ...
                if hasattr(tracker, "submit"):
                    state = tracker.submit(frame, now)
                else:
                    state = tracker.update(frame, now)
                state = grammar.update(state)

                release_energy = _decay(release_energy, dt, 4.5)
                shield_energy = _decay(shield_energy, dt, 2.2)
                ascension_energy = _decay(ascension_energy, dt, 2.0)
                portal_energy = _decay(portal_energy, dt, 1.6)
                ...
                for event in state.gestures:
                    if event.name == "charge_orb":
                        ...
                        shield_energy = max(shield_energy, event.strength)
                    elif event.name == "ascension_aura" and event.phase != "release":
                ...
                right = state.anchor("right_palm", 0.35)
                if charge > 0.0 and left and right:
                    cx = (left.position.x + right.position.x) * 0.5
                    cy = (left.position.y + right.position.y) * 0.5
                    emitters.insert(
                        0,
                        ParticleEmitter(cx, cy, 0.0, -0.04, 0.8 + charge * 0.7, 0.93, 0.025 + 0.030 * charge),
                    )
                if portal_emitters:
                    emitters = portal_emitters[:8]
                emitters = emitters[:8]

                motion_energy = 0.0
                if emitters:
                    motion_energy = float(np.mean([math.hypot(e.vx, e.vy) for e in emitters]))
                macro = max(release_energy, portal_energy, ascension_energy)
                emission = (
                    3200.0
                    + 5200.0 * min(motion_energy * 1.8, 1.0)
                    + 6200.0 * charge
                    + 15000.0 * release_energy
                    + 9000.0 * slash_energy
                    + 10000.0 * portal_energy
                ) * (0.70 + 0.65 * args.intensity)
                ...
                fx = field.render(
                    ...
                    drag=0.95 + 0.30 * (1.0 - args.madness),
                    feedback=feedback,
                    bloom=bloom,
                    energy=0.9 + 0.45 * args.intensity + 0.4 * macro,
                    bass=charge,
                ...
                if now - report >= 2.0:
                    ...
                    print(
                        f"[performer-fx] fps={frames/(now-report):.1f} source={state.source} "
                        f"anchors={len(state.anchors)} conf={state.performer_confidence:.2f} "
                        f"emitters={len(emitters)} gestures={gestures} "
                        f"charge={charge:.2f} release={release_energy:.2f} portal={portal_energy:.2f}",
                        flush=True,
                    )
                    report = now
                    frames = 0
